...
public class Uno {
    ...
    vog(phrase, index); //Percentual de vogais
    con(phrase, index); //Consoantes
    pvs(phrase, index); //Percentuais de vogais singulares
    npa(phrase, index); //Número de palavras
    }
    ...
    public static void pvs (String phrase, int index){
        ...
        System.out.print("Percentual de vogais:");
        if (index > 0){
            index--;
            ...
            if (letter.equalsIgnoreCase("a")) //Se for igual
                individual[0] ++; //Incremento na posição da letra
    ...
    public static void npa (String phrase, int index) {
        ...
        while (index > 1){ //Usado para garantir que todas as posições serão atingidas com veri e veri2
            ...
            if (veri != 32 && veri2 == 32) //Se a palavra possuir um caractere diferente do espaço próxima a sua posição
                sheep++; //Contador cresce
        }
